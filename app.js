...
const routes = require('./public/data/routes.json')
// *********************************************************** //
...
app.get('/upsertRoutes',
  async (req,res,next) => {
    ...
    for (route of routes){ 
      ...
      const direction_destination = route.attributes.direction_names[1];
      const direction_origin_stop = route.attributes.direction_destinations[0];
...
app.post('/routes/byType',
  async (req,res,next) => {
    const {type} = req.body;
    const routes = await Route.find({description:type})
    res.locals.routes = routes
    res.render('routeslist')
  }
)

app.post('/routes/byDestination',
  async (req,res,next) => {
    const {destination} = req.body;
    const routes = await Route.find({direction_destination_stop:destination})
    res.locals.routes = routes
    res.render('routeslist')
  }
)
